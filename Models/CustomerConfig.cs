using System;
using System.Collections.Generic;
using System.Text.Json.Serialization;

namespace PowerPlatformDashboard.Models
{
    public class CustomerConfig
    {
        public string CustomerId { get; set; }
        public string CustomerName { get; set; }
        public string TenantId { get; set; }
        public string ClientId { get; set; }
        public string ClientSecret { get; set; }
        public DateTime OnboardedDate { get; set; }
        public string OnboardingStatus { get; set; } // "Pending", "Active"
    }

    public class EnvironmentInfo
    {
        public string Id { get; set; }
        public string Name { get; set; }
        public string DisplayName { get; set; }
        public string Type { get; set; }
        public string Region { get; set; }
        public string State { get; set; }
        public DateTime CreatedTime { get; set; }
        public string CapacityJson { get; set; }
    }

    public class CapacityInfo
    {
        public string EnvironmentId { get; set; }
        public string EnvironmentName { get; set; }
        
        public double DatabaseConsumedGb { get; set; }
        public double DatabaseAllocatedGb { get; set; }
        public double FileConsumedGb { get; set; }
        public double FileAllocatedGb { get; set; }
        public double LogConsumedGb { get; set; }
        public double LogAllocatedGb { get; set; }
        
        public int DatabasePercentUsed => DatabaseAllocatedGb > 0 ? 
            (int)((DatabaseConsumedGb / DatabaseAllocatedGb) * 100) : 0;
        public int FilePercentUsed => FileAllocatedGb > 0 ? 
            (int)((FileConsumedGb / FileAllocatedGb) * 100) : 0;
        public int LogPercentUsed => LogAllocatedGb > 0 ? 
            (int)((LogConsumedGb / LogAllocatedGb) * 100) : 0;
    }

    public class PowerPagesScanReport
    {
        public string EnvironmentId { get; set; }
        public string WebsiteId { get; set; }
        public string WebsiteName { get; set; }
        public double? Score { get; set; }
        public int? FindingsCount { get; set; }
        public DateTime? CompletedOn { get; set; }
        public string RawJson { get; set; }
    }

    public class EnvironmentOperation
    {
        public string OperationId { get; set; }
        public string EnvironmentId { get; set; }
        public string Type { get; set; }
        public string State { get; set; }
        public DateTime Started { get; set; }
        public DateTime Completed { get; set; }
        public string RawJson { get; set; }
    }

    public class StorageThreshold
    {
        [JsonPropertyName("thresholdInMB")]
        public int ThresholdInMB { get; set; }

        [JsonPropertyName("warningMessageConstKey")]
        public string WarningMessageConstKey { get; set; }
    }

    public class StorageWarning
    {
        [JsonPropertyName("storageCategory")]
        public string StorageCategory { get; set; }

        [JsonPropertyName("storageEntity")]
        public string StorageEntity { get; set; }

        [JsonPropertyName("thresholds")]
        public List<StorageThreshold> Thresholds { get; set; }

        [JsonPropertyName("isActive")]
        public bool IsActive { get; set; }
    }

	public class DashboardData
	{
		public List<EnvironmentInfo> Environments { get; set; } = new();
		public List<AdvisorRecommendation> AdvisorRecommendations { get; set; } = new();

		public List<CapacityInfo> CapacityData { get; set; } = new();
		public List<PowerPagesScanReport> ScanReports { get; set; } = new();
		public List<StorageWarning> StorageWarnings { get; set; } = new();
		public List<EnvironmentOperation> EnvironmentOperations { get; set; } = new();
		public bool IsLoading { get; set; }
		public string ErrorMessage { get; set; }
	}
		
		   public class AdvisorRecommendationPage
    {
        [JsonPropertyName("nextLink")]
        public string? NextLink { get; set; }

        [JsonPropertyName("value")]
        public List<AdvisorRecommendation> Value { get; set; } = new();
    }

    public class AdvisorRecommendation
    {
        // e.g. "unusedApps", "capacityHotspots", etc.
        [JsonPropertyName("scenario")]
        public string? Scenario { get; set; }

        [JsonPropertyName("details")]
        public AdvisorRecommendationDetails Details { get; set; } = new();
    }

    public class AdvisorRecommendationDetails
    {
        [JsonPropertyName("expectedNextRefreshTimestamp")]
        public DateTime? ExpectedNextRefreshTimestamp { get; set; }

        [JsonPropertyName("lastRefreshedTimestamp")]
        public DateTime? LastRefreshedTimestamp { get; set; }

        [JsonPropertyName("resourceCount")]
        public int? ResourceCount { get; set; }
    }
}